...
function Progress() {
  const progressData = data.productRequests
    .filter((feedback) => feedback.status === "in-progress")
    .map((response, index) => (
      <div
        key={index}
        className="bg-white mt-5 py-[1.5em] px-[1.3em] rounded-lg border-t-[7px] border-[#AD1FEA]"
      >
        <div>
          <span className="circle inline-block h-[10px] w-[10px] rounded-[50%] bg-[#AD1FEA]"></span>
  ...
  return (
    <div className="px-[1.5em] py-2">
      ...
      <div className="mb-[2em]">{progressData}</div>
    </div>
  );
}
