...
function Live() {
  const liveData = data.productRequests
    .filter((feedback) => feedback.status === "live")
    .map((response, index) => (
      <Link to={`/feedbackdetails/${response.id}`} key={index}>
        <div
          key={index}
          className="bg-white mt-5 py-[1.5em] px-[1.3em] rounded-lg border-t-[7px] border-[#4661E6]"
        >
          <div>
            <span className="circle inline-block h-[10px] w-[10px] rounded-[50%] bg-[#4661E6]"></span>
          ...
          <p className="text-[#4661E6] bg-[#f7f8fd] font-bold rounded-md text-[.9em] cursor-pointer w-[120px] h-[37px] justify-center flex items-center mt-4">
            {response.category}
          </p>
  ...
  return (
    <div className="px-[1.5em] py-2 md:p-0">
      ...
      <p className="mt-3 text-[#647196]">Released features</p>

      <div className="mb-[2em]">{liveData}</div>
    </div>
  );
}
